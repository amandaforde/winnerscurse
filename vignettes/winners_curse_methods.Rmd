...
- Inspired by the winner's curse adjustment method detailed in [Faye *et al.* (2011)](https://onlinelibrary.wiley.com/doi/abs/10.1002/sim.4228), the function `out_BR_ss` implements a bootstrap method which is suitable for use with summary statistics. However, this approach is much more computationally intensive than all three methods described above.

**describe the method**: 

$$\hat\beta_{\text{boot}(k)} = \hat\beta_{\text{N}(k)} - \text{se}(\hat\beta_{(k)}) \cdot \frac{1}{N_{(k)}} \sum^{N_{(k)}}_{i=1} \frac{\hat\beta_{\text{b}i(k)} - \hat\beta_{\text{oob}i(k)}}{\text{se}(\hat\beta_{i(k)})}$$

- 100 bootstraps are used. 
