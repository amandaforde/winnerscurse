---
title: "Obtaining adjusted `beta` estimates"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Obtaining adjusted `beta` estimates}
...
- `BR_ss` takes a data frame `summary_data` with three columns: `rsid`, `beta`, `se`, and returns the inputted data frame along with the adjusted estimate for $\beta$, `beta_BR_ss`. The function also has a parameter `seed` which can be set to a specific numeric value by the user to permit reproducibility. This is required as this method involves bootstrap implementation and therefore, the use of a seed allows the user to obtain the exact same results for `beta_BR_ss` when running the function many times on the same dataset. The default value for `seed` is `1998`. 
...
- *Use of splines instead of loess perhaps in `BR_ss` - but how to choose appropriate value for df? Currently df=10 seems comparable to current implementation of `BR_ss`*
